...
private:
    ...
    int getAngle(int curByteNum, const QVector<int> &range);

    //выбирает число ветвей исходящих из узла
    int getNumberOfBranches(int curByteNum);

    //составляет дисретное распределение для работы функции getAngle
    void getAngleRange(QVector<int> &range);
    ...
    void getTrunkParams(QString &params, int curByteNum);
...
public:
    ...
    static const int m_endOfTrunkColorBlock = 169;
    ...
    void cross(const Genom &father, Genom& son);
};
